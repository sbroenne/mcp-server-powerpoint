...
description: 16 MCP tools with 190 operations across 16 domains for live PowerPoint automation through single action-dispatch tools.
...
PowerPoint MCP Server exposes **16 MCP tools with 190 operations across 16 domains**.
...
| `shape` | 50 | Shapes, styling, grouping, hyperlinks, linked pictures, placeholders, string tags | `shape(action="...", session_id=..., ...)` | `pptcli shape <action> -s <SESSION_ID> ...` |
...
Use `shape` for shape creation, WordArt, geometry, styling, 3D effects, grouping, naming, alt text,
hyperlinks, linked pictures, and placeholders.
...
`set-line`, `get-line`, `set-rotation`, `get-rotation`, `set-3d-rotation`, `get-3d-rotation`,
`flip`, `set-z-order`, `set-shadow`, `get-shadow`, `set-glow`, `get-glow`, `set-reflection`,
