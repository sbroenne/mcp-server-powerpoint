...
- Steps to reproduce (a minimal PowerPoint file/tool-call sequence, if
  applicable)
...
- **Local, Windows-only automation.** The MCP server drives a real, local
  PowerPoint desktop process over COM
  (`Microsoft.Office.Interop.PowerPoint`). It does not open network ports,
  does not send presentation content anywhere, and only operates on files
  you explicitly open, create, or export.
- **Native Office API only.** Because it uses PowerPoint's own COM
  automation surface rather than a third-party `.pptx` parser, there is no
  custom file-format parsing that could be exploited by a malformed file —
  PowerPoint itself validates and opens the file.
