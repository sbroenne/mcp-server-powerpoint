...
<#
.SYNOPSIS
    Checks that all ((dynamic)) casts in PowerPointMcp.Core and PowerPointMcp.ComInterop have
...
.DESCRIPTION
    ...
    Valid comment prefixes (on the line immediately before the cast):
      // PIA gap: ...    — Type not in the referenced Microsoft.Office.Interop.PowerPoint PIA
    ...
    False positives are excluded:
      - PresentationBatch.cs / PresentationSession.cs / PresentationSessionRegistry.cs /
        PresentationShutdownService.cs (infrastructure — uses `dynamic app`)
...
.NOTES
    Ported from mcp-server-excel's scripts/check-dynamic-casts.ps1.
...
$searchDirs = @(
    (Join-Path $rootDir "src\PowerPointMcp.Core"),
    (Join-Path $rootDir "src\PowerPointMcp.ComInterop")
)
...
$excludeFiles = @(
    ...
    "PresentationSessionRegistry.cs",
    "PresentationShutdownService.cs"
)
...
Write-Host '  // PIA gap: <type> not in the referenced PowerPoint PIA because...' -ForegroundColor Gray
...
Write-Host ""
Write-Host "Fix these before committing." -ForegroundColor Red
exit 1
