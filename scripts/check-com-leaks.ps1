...
# Ported from mcp-server-excel's scripts/check-com-leaks.ps1
...
Get-ChildItem -Path (Join-Path $rootDir "src") -Recurse -Filter "*.cs" | ForEach-Object {
    ...
    $isSessionFile = $_.FullName -match "PresentationBatch\.cs|PresentationSession\.cs|PresentationSessionRegistry\.cs|PresentationShutdownService\.cs"
