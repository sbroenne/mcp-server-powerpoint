...
<#
.SYNOPSIS
    Checks for Success = true with ErrorMessage violations (Rule 1)
...
.NOTES
    Ported from mcp-server-excel's scripts/check-success-flag.ps1.
    Part of pre-commit validation. See critical-rules.instructions.md Rule 1.
#>

$ErrorActionPreference = "Stop"
$rootDir = Split-Path -Parent $PSScriptRoot

Write-Host "Checking for Success = true with ErrorMessage violations (Rule 1)..." -ForegroundColor Cyan
Write-Host ""

$violations = @()

# Scan all Core Commands files (PowerPoint's Core layer has one folder per domain,
# e.g. src/PowerPointMcp.Core/Chart, .../Slide — unlike Excel's single Commands/ folder)
Get-ChildItem -Path "$rootDir\src\PowerPointMcp.Core" -Filter "*.cs" -Recurse | ForEach-Object {
...
Write-Host "  3. See critical-rules.instructions.md Rule 1 for pattern examples" -ForegroundColor Yellow
Write-Host ""
Write-Host "See: .github/instructions/critical-rules.instructions.md Rule 1" -ForegroundColor Gray
