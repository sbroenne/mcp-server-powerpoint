    ...
    public static ServiceInfo? ExtractServiceInfo(INamedTypeSymbol interfaceSymbol)
    {
        ...
        foreach (var member in interfaceSymbol.GetMembers())
        {
            if (member is IMethodSymbol method && method.MethodKind == MethodKind.Ordinary)
            {
                ...
                var hasBatchParameter = method.Parameters.Any(p => p.Type.Name == "IPresentationBatch");
                var hasProgressParameter = method.Parameters.Any(p => p.Type.Name == "IProgress");
                var parameters = method.Parameters
                    .Where(p => p.Type.Name != "IPresentationBatch" && p.Type.Name != "IProgress") // Skip batch and progress parameters
