...
namespace Sbroenne.PowerPointMcp.Generators;
...
public class ServiceRegistryGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateServiceRegistry(ServiceInfo info)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace Sbroenne.PowerPointMcp.Generated;");
        sb.AppendLine();
        ...
        sb.AppendLine("        {");
        GenerateSuppliedParameterValidationCall(sb, allExposedParams, "            ");
        sb.AppendLine("            return action switch");
        sb.AppendLine("            {");
        foreach (var method in info.Methods)
        {
    ...
    private static void GenerateCliRouteMethod(StringBuilder sb, ServiceInfo info)
    {
        ...
        sb.AppendLine();
        GenerateSuppliedParameterValidationCall(sb, allExposedParams, "            ", "action");
        sb.AppendLine();
        ...
        foreach (var method in info.Methods)
        {
            ...
            foreach (var p in method.Parameters.Where(p => !p.IsFileOrValue && IsRequiredParameter(p) && StringHelper.IsStringType(p.TypeName)))
            {
                var paramName = p.IsFromString && p.IsEnum ? (p.ExposedName ?? p.Name) : p.Name;
                if (p.AllowsEmptyString)
                {
                    sb.AppendLine($"                    if ({paramName} is null)");
                    sb.AppendLine($"                        throw new System.ArgumentException(\"{paramName} is required for {method.ActionName} action.\", \"{paramName}\");");
                }
                else
                {
                    sb.AppendLine($"                    Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty({paramName}, \"{paramName}\", \"{method.ActionName}\");");
                }
            }
            foreach (var p in method.Parameters.Where(p => p.IsEnum && IsRequiredParameter(p)))
            {
                var paramName = p.ExposedName ?? p.Name;
                sb.AppendLine($"                    Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty({paramName}, \"{paramName}\", \"{method.ActionName}\");");
            }
            ...
            {
                sb.AppendLine($"                    var resolved{StringHelper.ToPascalCase(p.Name)} = Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.ResolveFileOrValue({p.Name}, {p.Name}{p.FileSuffix});");
                // Validate required FileOrValue parameters AFTER resolution
                if (IsRequiredParameter(p))
                {
                    sb.AppendLine($"                    Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty(resolved{StringHelper.ToPascalCase(p.Name)}, \"{p.Name}\", \"{method.ActionName}\");");
                }
            }
    ...
    private static void GenerateRouteFromSettings(StringBuilder sb, ServiceInfo info, List<ExposedParameter> allExposedParams)
    {
        ...
        for (int i = 0; i < allExposedParams.Count; i++)
        {
            ...
            {
                sb.AppendLine($"                {p.Name}: Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.ParseTimeSpanOrSeconds(settings.{StringHelper.ToPascalCase(p.Name)}, \"{p.Name}\"){comma}");
            }
            else
            {
        ...
        List<ExposedParameter> allExposedParams)
    {
        ...
        foreach (var method in info.Methods)
        {
            var directNames = method.Parameters
                .Select(parameter => parameter.ExposedName ?? parameter.Name)
                .Distinct(StringComparer.Ordinal)
                .Select(name => $"\"{name}\"")
                .ToArray();
            var fileNames = method.Parameters
                .Where(parameter => parameter.IsFileOrValue)
                .Select(parameter => $"\"{parameter.ExposedName ?? parameter.Name}{parameter.FileSuffix}\"")
                .ToArray();
            if (fileNames.Length == 0)
            {
                sb.AppendLine($"                \"{method.ActionName}\" => new string[] {{ {string.Join(", ", directNames)} }},");
            }
            else
            {
                sb.AppendLine($"                \"{method.ActionName}\" => allowFileParameters");
                sb.AppendLine($"                    ? new string[] {{ {string.Join(", ", directNames.Concat(fileNames))} }}");
                sb.AppendLine($"                    : new string[] {{ {string.Join(", ", directNames)} }},");
            }
        }
        sb.AppendLine("                _ => throw new System.ArgumentException($\"Unknown action: {action}\")");
        sb.AppendLine("            };");
        sb.AppendLine("            var validSet = new System.Collections.Generic.HashSet<string>(validParameters, System.StringComparer.Ordinal);");
        sb.AppendLine("            var invalid = suppliedParameters.Where(parameter => !validSet.Contains(parameter)).Distinct(System.StringComparer.Ordinal).OrderBy(parameter => parameter, System.StringComparer.Ordinal).ToArray();");
        sb.AppendLine("            if (invalid.Length > 0)");
        sb.AppendLine("                throw new System.ArgumentException($\"Parameter(s) {string.Join(\", \", invalid)} are not valid for action '{action}'.\");");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        var canonicalNames = allExposedParams.Select(parameter => $"\"{parameter.Name}\"");
        sb.AppendLine($"            var canonicalPropertyNames = new System.Collections.Generic.HashSet<string>(new string[] {{ {string.Join(", ", canonicalNames)} }}, System.StringComparer.Ordinal);");
        sb.AppendLine("            var invalidPropertyNames = rawPropertyNames.Where(parameter => !canonicalPropertyNames.Contains(parameter)).Distinct(System.StringComparer.Ordinal).OrderBy(parameter => parameter, System.StringComparer.Ordinal).ToArray();");
        sb.AppendLine("            if (invalidPropertyNames.Length > 0)");
        sb.AppendLine("                throw new System.ArgumentException($\"Unknown or non-canonical parameter(s): {string.Join(\", \", invalidPropertyNames)}.\");");
        ...
        if (validatedMethods.Count > 0)
        {
            ...
            foreach (var method in validatedMethods)
            {
                             ...
                             !parameter.AllowsEmptyString))
                {
                    var propertyName = parameter.ExposedName ?? parameter.Name;
                    sb.AppendLine(
                        $"                    Sbroenne.PowerPointMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty(args.{StringHelper.ToPascalCase(propertyName)}, \"{propertyName}\", \"{method.ActionName}\");");
                }
                foreach (var parameter in method.Parameters.Where(parameter =>
                             !parameter.IsEnum &&
                             !StringHelper.IsStringType(parameter.TypeName) &&
                ...
                {
                    var propertyName = parameter.ExposedName ?? parameter.Name;
                    sb.AppendLine($"                    if (args.{StringHelper.ToPascalCase(propertyName)} is null)");
                    sb.AppendLine(
                        $"                        throw new System.ArgumentException(\"{propertyName} is required for {method.ActionName} action.\", \"{propertyName}\");");
                }
                foreach (var parameter in method.Parameters.Where(parameter => parameter.IsEnum))
                {
                    var propertyName = parameter.ExposedName ?? parameter.Name;
                    ...
                    {
                        sb.AppendLine(
                            $"                    if ({propertyExpression} is not null) ServiceRegistry.ParseEnumValue<{parameter.EnumTypeName}>({propertyExpression}, default, \"{propertyName}\"{BuildEnumAliasArguments(parameter)});");
                    }
                }
                sb.AppendLine("                    break;");
                sb.AppendLine("                }");
    ...
    private static bool IsRequiredParameter(ParameterInfo parameter)
        => parameter.IsRequired || (!parameter.HasDefault && !parameter.TypeName.EndsWith("?", StringComparison.Ordinal));
    ...
    private static string GenerateComparisonFile(ServiceInfo info)
    {
        ...
        foreach (var method in info.Methods)
        {
            ...
            {
                sb.AppendLine($"  {info.CategoryPascal}Action.{enumValue} => ToolsBase.ForwardToService(\"{info.Category}.{method.ActionName}\", sessionId),");
            }
        }
        sb.AppendLine();
    ...
    private static string GenerateSkillManifest(List<ServiceInfo> categories)
    {
        ...
        sb.AppendLine("/// Used by PowerPointMcp.Build.Tasks to generate SKILL.md files from templates.");
        sb.AppendLine("/// </summary>");
    ...
    // PowerPointMcp.Generators.Shared and included as source files
    ...
    private static string GenerateDispatchHelpers()
    {
        ...
        sb.AppendLine("        UnmappedMemberHandling = System.Text.Json.Serialization.JsonUnmappedMemberHandling.Disallow,");
        sb.AppendLine("        Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter(namingPolicy: null, allowIntegerValues: false) }");
        sb.AppendLine("    };");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Requires a named raw JSON argument to be present as a string; an empty string is valid.</summary>");
    ...
    /// replacing hand-written Handle*CommandAsync methods in PowerPointMcpService.
    /// </summary>
    private static string GenerateServiceDispatch(ServiceInfo info, string interfaceFullName)
    {
        ...
        sb.AppendLine($"            {info.CategoryPascal}Action action,");
        if (!info.NoSession)
            sb.AppendLine("            Sbroenne.PowerPointMcp.ComInterop.Session.IPresentationBatch batch,");
        sb.AppendLine("            string? argsJson)");
        sb.AppendLine("        {");
    ...
    private static void GenerateDispatchCase(StringBuilder sb, ServiceInfo info, MethodInfo method)
    {
        ...
        // Only pass batch if the method actually has an IPresentationBatch parameter
        ...
        if (method.HasProgressParameter)
            callArgs.Add("Sbroenne.PowerPointMcp.ComInterop.ProgressContext.Current");
    ...
    private static string BuildEnumAliasArguments(ParameterInfo parameter)
    {
        ...
        return ", " + string.Join(
            ...
            parameter.EnumAliases.Select(alias =>
                $"(\"{alias.Alias.Replace("\\", "\\\\").Replace("\"", "\\\"")}\", {parameter.EnumTypeName}.{alias.MemberName})"));
    }
