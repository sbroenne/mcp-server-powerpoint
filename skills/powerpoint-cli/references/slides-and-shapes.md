...
| `shape` | `set-3d-rotation` | `session_id`, `slide_index`, `shape_index`, optional `rotation_x`/`rotation_y`/`rotation_z` | Sets one or more axes of 3D rotation; omitted axes remain unchanged. X and Y must be between -90 and 90 degrees. Z rotates the shape around its Z axis and is tracked separately from the shape's 2D `rotation`. Returns all three axes. |
