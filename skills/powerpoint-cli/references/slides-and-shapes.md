...
| `shape` | `set-3d-rotation` | `session_id`, `slide_index`, `shape_index`, optional `rotation_x`/`rotation_y`/`rotation_z` | Sets one or more axes of 3D rotation; omitted axes remain unchanged. X and Y must be between -90 and 90 degrees. Z controls the effects camera and is independent of the shape's 2D rotation. Returns all three axes. |
| `shape` | `get-3d-rotation` | `session_id`, `slide_index`, `shape_index` | Returns `rotationX`, `rotationY`, and the effects-camera `rotationZ` in degrees. |
